...
exports.getBlockingNotification = exports.AUTOPILOT_LIVE_WEEKDAY_INDEX = exports.SIMULATION_DAY_START_TIME = exports.createGameSaveFromLoadedScenario = void 0;
exports.isoDatePart = isoDatePart;
exports.isoTimePart = isoTimePart;
exports.combineIsoDateTime = combineIsoDateTime;
exports.createNewGameSaveFromScenario = createNewGameSaveFromScenario;
exports.getBlockingNotificationForSave = getBlockingNotificationForSave;
exports.buildLiveReportNotificationBody = buildLiveReportNotificationBody;
exports.hasPendingEventsToday = hasPendingEventsToday;
exports.archiveAndResolveManagedLivesForDate = archiveAndResolveManagedLivesForDate;
exports.acknowledgeInboxNotification = acknowledgeInboxNotification;
exports.advanceOneDayLegacy = advanceOneDayLegacy;
exports.advanceOneDay = advanceOneDay;
const gameSaveSchema_1 = require("../save/gameSaveSchema");
Object.defineProperty(exports, "createGameSaveFromLoadedScenario", { enumerable: true, get: function () { return gameSaveSchema_1.createGameSaveFromLoadedScenario; } });
const inbox_1 = require("../save/inbox");
Object.defineProperty(exports, "getBlockingNotification", { enumerable: true, get: function () { return inbox_1.getBlockingNotification; } });
const financeSystem_1 = require("./financeSystem");
const idolStatusSystem_1 = require("./idolStatusSystem");
const livePerformanceWeb_1 = require("./livePerformanceWeb");
const liveScheduleWeb_1 = require("./liveScheduleWeb");
const scenarioRuntimeWeb_1 = require("./scenarioRuntimeWeb");
const scoutWeb_1 = require("./scoutWeb");
const scoutWeb_2 = require("./scoutWeb");
const songStatusSystem_1 = require("./songStatusSystem");
const monthlyLiveScheduler_1 = require("./monthlyLiveScheduler");
const mediaEventWeb_1 = require("./mediaEventWeb");
exports.SIMULATION_DAY_START_TIME = "08:00:00";
function isoDatePart(isoLike) {
    return String(isoLike ?? "").split("T")[0] || "2020-01-01";
}
function isoTimePart(isoLike) {
    const text = String(isoLike ?? "");
    const time = text.includes("T") ? text.split("T")[1] ?? "" : "";
    const m = /^(\d{2}:\d{2})(?::\d{2})?/.exec(time);
    return m?.[1] ?? "08:00";
}
function combineIsoDateTime(dateIso, hhmmss) {
    return `${isoDatePart(dateIso)}T${hhmmss}`;
}
function isoToComparableMs(isoLike) {
    const raw = String(isoLike ?? "").trim();
    if (!raw)
        return 0;
    const normalized = raw.includes("T") ? raw : combineIsoDateTime(raw, exports.SIMULATION_DAY_START_TIME);
    const parsed = Date.parse(`${normalized.endsWith("Z") ? normalized : `${normalized}Z`}`);
    return Number.isFinite(parsed) ? parsed : 0;
}
function keepCurrentDateMonotonic(save, previousIso, nextIso) {
    const prevText = currentSimulationIso({ ...save, current_date: previousIso ?? save.current_date });
    const nextText = String(nextIso ?? "").trim()
        ? (String(nextIso).includes("T") ? String(nextIso) : combineIsoDateTime(String(nextIso), exports.SIMULATION_DAY_START_TIME))
        : prevText;
...
function isoTimeToMinutes(isoLike) {
    return hhmmToMinutes(isoTimePart(isoLike));
}
function currentSimulationIso(save) {
    const base = save.current_date ?? save.game_start_date ?? save.scenario_context.startup_date ?? "2020-01-01";
    return String(base).includes("T") ? String(base) : combineIsoDateTime(String(base), exports.SIMULATION_DAY_START_TIME);
}
/** Legacy weekly autopilot marker retained only for older UI references. */
exports.AUTOPILOT_LIVE_WEEKDAY_INDEX = 3;
function deepSaveCopy(save) {
    return JSON.parse(JSON.stringify(save));
}
/** New desktop-shaped save from a loaded scenario (full DB trio). */
function createNewGameSaveFromScenario(loaded, opts) {
    const save = (0, gameSaveSchema_1.createGameSaveFromLoadedScenario)(loaded, {
        playerName: opts.playerName,
        managedGroupLabel: opts.managedGroupLabel,
        managedGroupUid: opts.managedGroupUid ?? null,
    });
    (0, monthlyLiveScheduler_1.ensureAutoBookedLivesThroughEndOfNextMonth)(save);
    (0, gameSaveSchema_1.refreshStartupUpcomingLivesNotification)(save, save.current_date ?? save.game_start_date ?? loaded.preset.opening_date ?? "2020-01-01");
    save.current_date = combineIsoDateTime(save.current_date ?? save.game_start_date ?? loaded.preset.opening_date ?? "2020-01-01", exports.SIMULATION_DAY_START_TIME);
    seedTodaysLiveBlockingInbox(save, save.current_date ?? save.game_start_date ?? loaded.preset.opening_date ?? "2020-01-01");
    (0, monthlyLiveScheduler_1.maybeSeedMonthEndAutoBookPrompt)(save);
    return save;
}
function memberCountFromSave(save) {
    const g = (0, gameSaveSchema_1.getPrimaryGroup)(save);
...
function getBlockingNotificationForSave(save) {
    const cur = save.current_date ?? save.game_start_date ?? save.scenario_context.startup_date ?? "2020-01-01";
    return (0, inbox_1.getBlockingNotification)(save.inbox.notifications, String(cur));
}
function formatTodaysLiveScheduleBody(lives, members) {
    if (!lives.length)
        return "";
    const lines = [];
    ...
    lives.forEach((live, i) => {
        ...
        const setlist = Array.isArray(live.setlist) ? live.setlist.map((x) => String(x)) : [];
        ...
        const slot = (0, liveScheduleWeb_1.formatLiveSlotLine)(live);
        lines.push(`\n— ${i + 1}. ${title}`);
...
function buildLiveReportNotificationBody(live) {
    const memberLines = [];
    ...
    if (Array.isArray(deltas)) {
        for (const row of deltas.slice(0, 6)) {
            ...
            memberLines.push(`${nm}: rate ${rate}, fans ${fg >= 0 ? "+" : ""}${fg}, morale ${mg >= 0 ? "+" : ""}${mg}, tokutenkai ${tk}`);
        }
    }
    ...
    const loc = String(live.location ?? "").trim();
    const when = (0, liveScheduleWeb_1.formatLiveSlotLine)(live) || String(live.start_date ?? "").split("T")[0];
    ...
    const tokutenkaiGross = Number(live.tokutenkai_revenue_yen ?? (0, livePerformanceWeb_1.estimateTokutenkaiRevenueYen)(tokutenkaiActual)) || 0;
    ...
    if (venue)
        body += ` Venue: ${venue}${loc ? ` (${loc})` : ""}.`;
    if (when)
        body += ` Slot: ${when}.`;
    const setlist = Array.isArray(live.setlist) ? live.setlist.map((x) => String(x)).filter(Boolean) : [];
    if (setlist.length)
        body += ` Setlist: ${setlist.join(" · ")}.`;
    if (tokutenkaiActual || tokutenkaiPlanned) {
        body += ` Tokutenkai ${tokutenkaiActual}/${tokutenkaiPlanned} tickets`;
        if (tokutenkaiGross > 0)
            body += ` (gross ¥${tokutenkaiGross.toLocaleString("ja-JP")})`;
        body += `.`;
    }
    if (ticketGross > 0 || goodsGross > 0) {
        const revenueBits = [];
        if (ticketGross > 0)
            revenueBits.push(`tickets ¥${ticketGross.toLocaleString("ja-JP")}`);
        if (goodsGross > 0)
            revenueBits.push(`goods ¥${goodsGross.toLocaleString("ja-JP")}`);
        body += ` Sales: ${revenueBits.join(" · ")}.`;
    }
    if (memberLines.length)
        body += " " + memberLines.join(" | ");
    return body;
}
function durationMinutesFromLive(live) {
    const start = String(live.start_time ?? "").slice(0, 5);
    const end = String(live.tokutenkai_enabled ? live.tokutenkai_end ?? live.end_time ?? "" : live.end_time ?? "").slice(0, 5);
    const parse = (value) => {
...
function liveReportEndTime(live) {
    ...
    if (/^\d{2}:\d{2}$/.test(liveEnd))
        return liveEnd;
    return "21:00";
}
function todaysLiveScheduleNotificationTime(lives) {
...
function availableManagedMemberUidsForDate(save, targetIso) {
    ...
    return rosterUids.filter((uid) => {
        ...
        return idol ? !(0, idolStatusSystem_1.isIdolOnHiatus)(idol, targetIso) : true;
    });
}
function activeManagedMembersForDate(save, targetIso) {
    const rosterUids = new Set(availableManagedMemberUidsForDate(save, targetIso));
    return save.database_snapshot.idols.filter((idol) => rosterUids.has(String(idol.uid ?? "")));
}
function collectTodaySimulationEvents(save) {
    ...
    const trainingBySlot = new Map();
    for (const uid of rosterUids) {
        const intensity = (0, idolStatusSystem_1.safeTrainingRow)(save.training_intensity[uid] ?? (0, idolStatusSystem_1.defaultAutopilotTrainingIntensity)());
        const plan = (0, idolStatusSystem_1.buildDailyTrainingPlan)(intensity, todayIso, liveDaysInWeek);
    ...
    for (const live of todaysLives) {
        ...
        if (isoTimeToMinutes(eventIso) <= nowMin)
            continue;
        out.push({
            kind: "live_start",
            iso: eventIso,
    ...
    for (const raw of save.lives.results) {
        if (!raw || typeof raw !== "object")
            continue;
        const live = raw;
        if (isoDatePart(String(live.start_date ?? live.date ?? "")) !== todayIso)
            continue;
        if (live.report_generated_same_day === true)
            continue;
        const reportIso = combineIsoDateTime(todayIso, `${liveReportNotificationTime(live)}:00`);
        if (isoTimeToMinutes(reportIso) <= nowMin)
            continue;
        out.push({
...
function hasPendingEventsToday(save) {
    return collectTodaySimulationEvents(save).length > 0;
}
function buildLiveReportData(live) {
    const ticketGross = Number(live.ticket_gross_yen ?? 0) || 0;
    const goodsGross = Number(live.goods_gross_yen ?? 0) || 0;
    const tokutenkaiRevenue = Number(live.tokutenkai_revenue_yen ?? (0, livePerformanceWeb_1.estimateTokutenkaiRevenueYen)(Number(live.tokutenkai_actual_tickets ?? 0) || 0)) || 0;
    return {
        kind: "live_report",
        ...
        slot: (0, liveScheduleWeb_1.formatLiveSlotLine)(live) || String(live.start_date ?? "").split("T")[0],
        ...
        member_deltas: Array.isArray(live.member_deltas)
            ? live.member_deltas.map((row) => {
                ...
                return {
                    ...r,
                    cheki_sale_money_yen: (0, livePerformanceWeb_1.estimateTokutenkaiRevenueYen)(tk),
                };
            })
            : [],
    };
}
function addLiveReportNotification(save, live, targetIso) {
...
function subtractBreakdowns(a, b) {
    const keys = [
        "income_total",
        "expense_total",
        "net_total",
...
function applyLiveFinanceSettlement(finances, p) {
    const base = (0, financeSystem_1.buildDailyBreakdown)({
    ...
    const full = (0, financeSystem_1.buildDailyBreakdown)({
        targetDateIso: p.targetIso,
        memberCount: p.memberCount,
        popularity: p.popularity,
        fans: p.fans,
    ...
    return (0, financeSystem_1.applyDailyClose)(finances, delta);
}
/**
 ...
 */
function archiveAndResolveManagedLivesForDate(save, targetIso) {
    const group = (0, gameSaveSchema_1.getPrimaryGroup)(save);
    ...
    const songs = save.database_snapshot.songs;
    const weekLog = (0, idolStatusSystem_1.normalizeTrainingWeekLog)(save.training_week_log);
    const resultUids = new Set(save.lives.results
        .map((raw) => {
        if (!raw || typeof raw !== "object")
            return "";
        const r = raw;
        ...
        .filter(Boolean));
    const remaining = [];
    let finances = (0, financeSystem_1.normalizeFinances)((0, gameSaveSchema_1.getActiveFinances)(save));
    const mc = memberCountFromSave(save);
    const letterTier = (0, gameSaveSchema_1.getLetterTierFromGroup)(group);
    const monthlySalaryTotal = mc * (0, financeSystem_1.monthlyBaseSalaryYenForGroupLetterTier)(letterTier);
    for (const raw of save.lives.schedules) {
        ...
        const resolution = (0, livePerformanceWeb_1.resolveGroupLiveResultWeb)(g, members, songs, live, save.managed_song_status);
        const applied = (0, livePerformanceWeb_1.applyLiveResultToSnapshot)(g, members, resolution);
        ...
        const memberDeltaByUid = new Map();
        ...
        for (const member of members) {
            ...
            (0, idolStatusSystem_1.applyDailyStatusUpdateJson)(member, {
                trainingLoad: 0,
                trainingHours: 0,
                liveCount: 1,
                liveMinutes,
        ...
        const managedGroup = (0, gameSaveSchema_1.getPrimaryGroup)(save);
        const goodsUids = Array.isArray(live.goods_uids)
            ? live.goods_uids.map((x) => String(x))
        ...
        const goodsGross = live.goods_enabled
            ? goodsUids.reduce((sum, goodsUid) => {
                ...
                if (!goods)
                    return sum;
                const goodsUnits = (0, financeSystem_1.estimateLiveGoodsUnits)(goods, {
                    liveType: String(live.live_type ?? ""),
                    capacity: Number(live.capacity ?? 0) || 0,
                    groupFans: Number(managedGroup?.fans ?? 0) || 0,
                    groupPopularity: Number(managedGroup?.popularity ?? 0) || 0,
                    groupTier: (0, gameSaveSchema_1.getLetterTierFromGroup)(managedGroup),
                });
                if (goodsUnits > 0) {
        ...
        const tokutenkaiRevenue = (0, livePerformanceWeb_1.estimateTokutenkaiRevenueYen)(resolution.tokutenkai_actual_tickets);
        const played = {
            ...live,
            status: "played",
            ...resolution,
        ...
        (0, songStatusSystem_1.registerManagedSetlistPerformance)(save.managed_song_status, songs, String(g.uid ?? ""), Array.isArray(live.setlist) ? live.setlist.map((x) => String(x)) : [], targetIso);
        ...
        const liveVenueFeeTotal = (0, financeSystem_1.estimateVenueFee)(cap, { isWeekendOrHoliday: (0, financeSystem_1.isWeekendUtc)(targetIso) });
    ...
    save.training_week_log = weekLog;
}
function scheduledManagedLivesForDate(save, targetIso) {
    const g = (0, gameSaveSchema_1.getPrimaryGroup)(save);
    const gid = g && String(g.uid ?? "");
    if (!gid)
        return [];
    return save.lives.schedules.filter((raw) => {
        if (!raw || typeof raw !== "object")
            return false;
        const live = raw;
        const sd = String(live.start_date ?? "").split("T")[0];
...
function acknowledgeInboxNotification(save, notificationUid) {
    ...
    if (dk.startsWith("auto-book-lives|")) {
        ...
        if (/^\d{4}-\d{2}-\d{2}$/.test(monthStart)) {
            (0, monthlyLiveScheduler_1.autoBookMonthFromMonthEndPrompt)(next, monthStart);
        }
    }
    item.read = true;
...
function applyMorningRecovery(next, targetDateIso) {
    const group = (0, gameSaveSchema_1.getPrimaryGroup)(next);
    if (!group || typeof group !== "object")
        return;
    const g = group;
    const memberUids = Array.isArray(g.member_uids)
        ? g.member_uids.map((x) => String(x))
        : [];
    const rosterUids = memberUids.length > 0 ? memberUids : next.shortlist.map((x) => String(x));
    const idols = next.database_snapshot.idols;
    for (const uid of rosterUids) {
        const idol = idols.find((r) => String(r.uid ?? "") === uid);
        if (!idol)
            continue;
        (0, idolStatusSystem_1.applyDailyStatusUpdateJson)(idol, {
            trainingLoad: 0,
            trainingHours: 0,
            liveCount: 0,
            liveMinutes: 0,
    ...
    (0, songStatusSystem_1.decayManagedSongsOvernight)(next.managed_song_status, next.training_song_uids, targetDateIso);
    (0, songStatusSystem_1.maybeAddSongUnlockNotification)(next, targetDateIso);
}
function processLiveReportEvent(next, event) {
    const dayIso = isoDatePart(event.iso);
    publishPendingLiveReportsForDate(next, dayIso);
}
function publishPendingLiveReportsForDate(next, dayIso) {
    let latestReportTime = null;
    for (const raw of next.lives.results) {
        if (!raw || typeof raw !== "object")
            continue;
        const live = raw;
        if (isoDatePart(String(live.start_date ?? live.date ?? "")) !== dayIso)
            continue;
        if (live.report_generated_same_day === true)
            continue;
        ...
        if (!latestReportTime || reportTime > latestReportTime)
            latestReportTime = reportTime;
    }
    return latestReportTime;
}
function processTrainingEndEvent(next, event) {
    const idols = next.database_snapshot.idols;
    const affected = [];
    ...
    const songUpdates = (0, songStatusSystem_1.applyTrainingToManagedSongs)(next.managed_song_status, next.training_song_uids, isoDatePart(event.iso), maxBlocks || 1);
    (0, inbox_1.addNotification)(next, {
...
function advanceOneDayLegacy(save) {
    const next = deepSaveCopy(save);
    const beforeIso = currentSimulationIso(next);
    (0, monthlyLiveScheduler_1.ensureAutoBookedLivesThroughEndOfNextMonth)(next);
    ...
    let finances = (0, financeSystem_1.normalizeFinances)((0, gameSaveSchema_1.getActiveFinances)(next));
    const scoutRetainersMonthlyTotal = (0, scoutWeb_2.totalMonthlyScoutRetainersYen)(next.scout.subscriptions, (0, scoutWeb_1.buildDefaultScoutCompanies)());
    const targetIso = (0, financeSystem_1.addCalendarDays)(currentDayIso, 1);
    ...
    let mediaSummary = (0, mediaEventWeb_1.resolveManagedMediaDay)(null, targetIso, null, [], letterTier, 0);
    if (group && typeof group === "object") {
        const g = group;
        ...
        const weekLog = (0, idolStatusSystem_1.normalizeTrainingWeekLog)(next.training_week_log);
        const liveDaysInWeek = new Set(next.lives.schedules
            .filter((raw) => Boolean(raw && typeof raw === "object"))
        ...
        for (const uid of rosterUids) {
            ...
            if (!ti || typeof ti !== "object") {
                next.training_intensity[uid] = { ...(0, idolStatusSystem_1.defaultAutopilotTrainingIntensity)() };
            }
            const intensity = (0, idolStatusSystem_1.safeTrainingRow)(next.training_intensity[uid]);
            const focus = String(next.training_focus_skill[uid] ?? "");
            const trainingPlan = (0, idolStatusSystem_1.buildDailyTrainingPlan)(intensity, targetIso, liveDaysInWeek);
            (0, idolStatusSystem_1.recordTrainingDay)(weekLog, uid, targetIso, intensity, trainingPlan.trainingHours, trainingPlan.sessionLabels, liveCount, liveMinutes, focus);
        }
        next.training_week_log = weekLog;
        const mediaBundle = (0, mediaEventWeb_1.findManagedOfficialScheduleBundleInRuntime)(next.scenario_runtime.official_schedules, g, next.managing_group);
        mediaSummary = (0, mediaEventWeb_1.resolveManagedMediaDay)(mediaBundle, targetIso, g, activeMembers, letterTier, 0);
    }
    const { popularity, fans, xFollowers } = readPopFans(next);
    const breakdown = (0, financeSystem_1.buildDailyBreakdown)({
    ...
    if (group && typeof group === "object") {
        ...
        for (const idol of idols) {
            ...
            if (!conditionDelta && !fanDelta && !moraleDelta)
                continue;
    ...
    finances = (0, financeSystem_1.applyDailyClose)(finances, breakdown);
    next.finances = finances;
    next.turn_number = (typeof next.turn_number === "number" ? next.turn_number : 0) + 1;
    next.current_date = combineIsoDateTime(targetIso, exports.SIMULATION_DAY_START_TIME);
    (0, scenarioRuntimeWeb_1.applyScenarioEventsForDate)(next, targetIso);
    (0, monthlyLiveScheduler_1.ensureAutoBookedLivesThroughEndOfNextMonth)(next);
    (0, gameSaveSchema_1.refreshStartupUpcomingLivesNotification)(next, targetIso);
    (0, monthlyLiveScheduler_1.maybeSeedMonthEndAutoBookPrompt)(next);
    if (!next.scout.selected_company_uid) {
        next.scout.selected_company_uid = (0, scoutWeb_1.buildDefaultScoutCompanies)()[0]?.uid ?? null;
    }
    if (next.inbox.notifications.length > 500) {
        next.inbox.notifications = next.inbox.notifications.slice(-500);
    }
...
function advanceOneDay(save) {
    ...
    dayAdvanced.current_date = combineIsoDateTime(targetIso, exports.SIMULATION_DAY_START_TIME);
