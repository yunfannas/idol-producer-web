...
const fallbackCsvPath = path.join(root, "docs", "scenario_6_recommended_group_idols.generated.csv");
...
for (const target of TARGET_GROUPS) {
  ...
  for (const { idol, relevant, forcedStatus } of groupRows) {
    ...
    if (hasPersistedAttributeBlock(idol.attributes)) {
      ...
      attrs = buildAttributesFromFollowerModel(idol, groupPopularity, openingDate);
