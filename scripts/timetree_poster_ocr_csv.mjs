...
function importCsv(csvPath, reviewPath) {
  ...
  if (!reviewPath) {
    ...
    reviewPath = path.join(root, "docs", "reference", "timetree_poster_ocr", `${base}.json`);
...
if (cmd === "export") {
  const reviewPath = path.resolve(
    fileArg ?? path.join(root, "docs/reference/timetree_poster_ocr/akishibu-poster-ocr-review.json"),
  );
  const outPath = outIdx >= 0 ? path.resolve(argv[outIdx + 1]) : undefined;
  exportReview(reviewPath, outPath);
} else if (cmd === "import") {
  const csvPath = path.resolve(
    fileArg ?? path.join(root, "docs/reference/timetree_poster_ocr/akishibu-poster-ocr-review.csv"),
  );
