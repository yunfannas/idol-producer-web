...
exports.deterministicNoise = deterministicNoise;
exports.collectRecentReleaseSignalFromSnapshot = collectRecentReleaseSignalFromSnapshot;
exports.resolveGroupLiveResultWeb = resolveGroupLiveResultWeb;
exports.applyLiveResultToSnapshot = applyLiveResultToSnapshot;
exports.estimateTokutenkaiRevenueYen = estimateTokutenkaiRevenueYen;
...
function isLimitedDiscoveryLiveType(liveType) {
    return liveType === "Taiban" || liveType === "Festival" || liveType === "Joint";
}
function avgFloat(...values) {
    const v = values.filter((x) => Number.isFinite(x));
    if (!v.length)
        return 0;
    return v.reduce((a, b) => a + b, 0) / v.length;
}
function hexDigestUtf8(message) {
    const b = (0, sha256sync_1.sha256BytesUtf8)(message);
...
function parseBirthdayParts(iso) {
...
function ageAtReference(birth, refIso) {
...
function idolAgeOn(idol, refIso) {
...
function idolTenureYears(idol, refIso) {
...
function salesAgeBonus(idol, refIso) {
    ...
    if (age < 17)
        ...
        bonus -= Math.min(0.25, (age - 24.0) * 0.05);
    return clamp(bonus, -0.4, 0.6);
}
function salesTenureAdjustment(idol, refIso) {
    const tenureYears = idolTenureYears(idol, refIso);
    const earlyBonus = Math.min(0.28, tenureYears * 0.05);
    let veteranPenalty = 0;
    if (tenureYears > 8.0)
        veteranPenalty = Math.min(0.55, (tenureYears - 8.0) * 0.08);
...
function isAkishibuGroup(group) {
    if (!group)
        return false;
    for (const key of groupIdentityKeys(group)) {
        if (key.includes("akishibu") || key.includes("アキシブ"))
            return true;
    }
    return false;
}
function akishibuRoleBias(group, idol) {
    if (!isAkishibuGroup(group))
        return { performance: 0, sales: 0 };
    const names = new Set([idol.uid, idol.name, idol.romaji].map((x) => String(x ?? "").trim().toLowerCase()).filter(Boolean));
    let performance = 0;
    let sales = 0;
...
function memberLiveComponentScores(idol, refIso) {
    const a = (0, idolAttributes_1.normalizePersistedAttributes)(idol.attributes);
...
function memberStatusMultiplier(idol) {
...
function memberMoodScore(idol) {
    const condition = num(idol.condition, 90);
    const morale = num(idol.morale, 70);
    return clamp(condition * 0.58 + morale * 0.42, 0, 100);
}
function memberConditionScore(idol) {
    return clamp(num(idol.condition, 90), 0, 100);
}
function memberFatigueScore(idol) {
    return clamp(100 - num(idol.condition, 90), 0, 100);
}
function idolLiveReadinessScore(idol, liveType, refIso) {
...
function memberTokutenkaiSalesScore(idol, live, group, refIso) {
    const comp = memberLiveComponentScores(idol, refIso);
    const a = (0, idolAttributes_1.normalizePersistedAttributes)(idol.attributes);
    const ment = a.mental;
    const app = a.appearance;
    ...
    const popularitySignal = avgFloat(num(idol.popularity, 0), Math.min(20.0, Math.log10(fc) * 4.8));
    let score = 5.9;
...
function idolLivePerformanceRating(idol, liveType, live, group, refIso) {
    const comp = memberLiveComponentScores(idol, refIso);
    ...
    let fitStrength;
    if (typeKey === "Concert")
        fitStrength = comp.vocal;
    else if (typeKey === "Festival")
        fitStrength = comp.stage;
    else if (typeKey === "Taiban")
        fitStrength = avgFloat(comp.stage, comp.dance);
    else
        fitStrength = avgFloat(comp.vocal, comp.stage);
    let rating = 6.0;
...
function collectRecentReleaseSignalFromSnapshot(songs, groupUid, liveDateIso, live) {
    ...
    for (const song of songs) {
        if (!song || typeof song !== "object")
            continue;
        const s = song;
        if (String(s.group_uid ?? "") !== String(groupUid))
            continue;
        if (s.hidden === true)
            continue;
        if ((0, songDisplayPolicy_1.isSongHiddenFromDisplay)(s, songs))
            continue;
        ...
        if (delta >= 0 && delta <= 60) {
            recentSongCount += 1;
            const title = (0, songCatalog_1.songCatalogDisplayLabel)(s);
            if (title)
    ...
    if (live.costume_refresh === true)
        costumeRefreshBonus += 3.0;
    costumeRefreshBonus += clamp(num(live.costume_refresh_level, 0), 0, 4);
    if (recentDiscCount > 0)
        costumeRefreshBonus += 1.0;
    const noveltyScore = Math.min(12.0, Math.min(recentSongCount, 3) * 1.5 +
...
function mergeDiscographySignal(group, live, liveDateIso, base) {
    const liveDate = parseIsoDate(liveDateIso);
    if (!liveDate)
        return base;
    const liveT = Date.parse(liveDate + "T12:00:00Z");
    ...
    if (recentDiscCount > 0)
        costumeRefreshBonus += 1.0;
    const noveltyScore = Math.min(12.0, Math.min(base.recent_song_count, 3) * 1.5 +
...
function estimateGroupFans(group, members) {
    const direct = Math.max(0, num(group.fans, 0));
    let memberTotal = 0;
    for (const m of members)
        memberTotal += Math.max(0, num(m.fan_count, 0));
    return Math.max(direct, memberTotal, 0);
}
function estimateGroupXFollowers(members) {
    let s = 0;
    for (const m of members)
        s += Math.max(0, num(m.x_followers, 0));
    return s;
}
function expectationScore(group, members, liveType, profileStrength, noveltyScore) {
...
function resolveGroupLiveResultWeb(group, members, songs, live, managedSongStatus) {
    const liveType = String(live.live_type ?? live.event_type ?? "Routine");
    const refIso = parseIsoDate(live.start_date) ?? (String(live.start_date ?? "").split("T")[0] || "2020-01-01");
    const memberScores = [];
    ...
    const rosterScore = memberScores.length > 0
        ? avgFloat(...memberScores.map((r) => num(r.score, 45)))
        : 45.0;
    const averageRating = memberScores.length > 0
        ? avgFloat(...memberScores.map((r) => num(r.rating, 5.5)))
        : 5.5;
    const synergy = members.length > 0
        ? avgFloat(...members.map((m) => memberLiveComponentScores(m, refIso).teamwork))
        : 10.0;
    ...
    const setlistEffect = (0, songStatusSystem_1.managedSetlistEffect)(managedSongStatus ?? {}, songs, gUid, Array.isArray(live.setlist) ? live.setlist.map((x) => String(x)) : []);
    ...
    if (capacity > 0) {
        const demandAnchor = clamp((profileStrength / 100.0) * 0.8 + (performanceScore / 100.0) * 0.42 + (noveltyScore / 100.0) * 0.15, 0.12, 1.0);
        ...
        const groupFans = estimateGroupFans(group, members);
        const xFollowers = estimateGroupXFollowers(members);
        attendance = Math.max(800, Math.round(900 + groupFans * 0.015 + xFollowers * 0.0025 + profileStrength * 18.0 + noveltyScore * 22.0));
        broadcastExposure = Math.max(500, Math.round(attendance * (0.55 + profileStrength / 180.0 + noveltyScore / 25.0)));
    }
    else {
        attendance = expectedTickets;
    }
    const audienceSatisfaction = clamp(performanceScore * 0.74 + profileStrength * 0.16 + noveltyScore * 1.1 + deterministicNoise(`audi:${live.uid}`) * 3.0, 20, 100);
    let tokutenkaiFactor = clamp(0.6 + (audienceSatisfaction / 100.0) * 0.7, 0.45, 1.25);
    const lineupSalesStrength = avgFloat(...memberScores.map((r) => num(r.tokutenkai_sales_score, 6.0)));
    const topSellerStrength = Math.max(...memberScores.map((r) => num(r.tokutenkai_sales_score, 6.0)), 6.0);
    ...
    if (expectedTickets > 0)
        actualTickets = Math.max(1, actualTickets);
    ...
    if (expectationGap >= 18 || audienceSatisfaction >= 82)
        popularityGain = 2;
    else if (expectationGap >= 6 || audienceSatisfaction >= 68)
        popularityGain = 1;
    else if (expectationGap <= -18 || audienceSatisfaction < 38)
        popularityGain = -2;
    else if (expectationGap <= -8 || audienceSatisfaction < 42)
        popularityGain = -1;
    return {
...
function applyLiveResultToSnapshot(group, members, liveResult) {
    ...
    for (const item of liveResult.member_scores) {
        ...
        if (typeof rt === "number" && Number.isFinite(rt))
            ratingByUid.set(u, rt);
    }
    const totalTokutenkaiTickets = Math.max(0, Math.trunc(num(liveResult.tokutenkai_actual_tickets, 0)));
    let groupMoraleGain = 0;
    if (performanceScore >= 86 || satisfaction >= 84)
        groupMoraleGain = 3;
    else if (performanceScore >= 74 || satisfaction >= 72)
        groupMoraleGain = 2;
    else if (performanceScore >= 62 || satisfaction >= 60)
        groupMoraleGain = 1;
    else if (performanceScore < 42 || satisfaction < 40)
        groupMoraleGain = -2;
    else if (performanceScore < 52 || satisfaction < 50)
        groupMoraleGain = -1;
    const memberWeights = [];
    let totalWeight = 0;
    for (const idol of members) {
        ...
        const weight = fanGain >= 0 ? Math.max(0.25, memberScore / 100.0) : Math.max(0.25, (120.0 - memberScore) / 100.0);
        memberWeights.push({ idol, weight });
        totalWeight += weight;
    }
    const tokutenkaiAlloc = new Map();
    ...
    memberWeights.forEach(({ idol, weight }, index) => {
        ...
        if (fanGain > 0 && memberFanGain <= 0)
            memberFanGain = index === 0 ? 1 : 0;
        else if (fanGain < 0 && memberFanGain >= 0)
            memberFanGain = index === 0 ? -1 : 0;
        ...
        if (satisfaction >= 84 && ms >= performanceScore)
            memberPopDelta = 1;
        else if (satisfaction < 40 && ms < 45)
            memberPopDelta = -1;
        idol.popularity = clamp(num(idol.popularity, 0) + memberPopDelta, 0, 100);
        let memberMoraleDelta = groupMoraleGain;
        if (ms >= performanceScore + 6)
            memberMoraleDelta += 1;
        else if (ms <= performanceScore - 8 && memberMoraleDelta > 0)
            memberMoraleDelta -= 1;
        idol.morale = clamp(num(idol.morale, 70) + memberMoraleDelta, 0, 100);
        const lr = ratingByUid.get(uid);
        const performance_rating = lr != null && Number.isFinite(lr)
...
function estimateTokutenkaiRevenueYen(actualTickets) {
    if (actualTickets <= 0)
        return 0;
