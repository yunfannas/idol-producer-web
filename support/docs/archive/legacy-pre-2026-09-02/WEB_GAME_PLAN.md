...
Selected modules are also mirrored under **`support/reference/python-desktop/`** for in-repo reading while porting; treat **`idol_producer/`** as canonical if the copies drift.

| Topic | Primary location (`idol_producer/`) | Mirror in repo (`support/reference/python-desktop/`) |
|--------|--------------------------------------|--------------------------------|
