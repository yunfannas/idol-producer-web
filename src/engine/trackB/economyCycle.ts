...
export function applyTrackBTrainingLoad(
  ...
  if (!m) return;
  const hours = Math.max(0.5, blocks * 4);
  const load = hours * 3;
  applyConditionCost(m, load);
  const tier = String(getLetterTierFromGroup(getPrimaryGroup(save)) ?? "D");
  const focus = intensity.sing >= intensity.dance && intensity.sing >= intensity.physical ? "breath" : intensity.dance >= intensity.physical ? "agility" : "stamina";
  for (const attribute of ["agility", "tone", "pitch", "breath", "rhythm", "power", "stage_presence", "stamina"] as const) {
    applyAttributeExp(m, attribute, hours, 1, tier, focus);
  }
  m.trait_xp.singer = Math.max(0, Number(m.trait_xp.singer ?? 0) + hours * Math.max(.2, intensity.sing / 10));
  m.trait_xp.dancer = Math.max(0, Number(m.trait_xp.dancer ?? 0) + hours * Math.max(.2, intensity.dance / 10));
  tb.staff.manager_ap_used_week += Math.ceil(hours * 2);
  tb.staff.coach_ap_used_week.vocal += Math.ceil(hours * 2);
  tb.staff.coach_ap_used_week.dance += Math.ceil(hours * 2);
  recoverCondition(m, .5);
  syncCondition(idol, m);
}

/** Staff's ordered Active Song List is serviced before discretionary growth. */
export function applyTrackBFormationRehearsal(save: GameSavePayload, hours: number): void {
  const tb = ensureTrackB(save);
  const active = tb.active_song_uids.filter((uid) => Boolean(tb.song_familiarity[uid]));
  if (!active.length || hours <= 0) return;
  const members = Object.values(tb.members);
  const attrs = members.flatMap((member) => [member.attributes.agility, member.attributes.pitch, member.attributes.tone, member.attributes.breath, member.attributes.rhythm, member.attributes.power, member.attributes.stage_presence, member.attributes.stamina]);
  const teamAttrAvg = attrs.reduce((sum, value) => sum + value, 0) / Math.max(1, attrs.length);
  const factor = .5 + .5 * teamAttrAvg / 14;
  let remaining = hours;
  for (const uid of active) {
    if (remaining <= 0) break;
    const allocated = Math.min(remaining, Math.max(.5, hours / active.length));
    const familiarity = tb.song_familiarity[uid]!;
    familiarity.formation = clamp(familiarity.formation + allocated * 6 * Math.max(.15, 1 - familiarity.formation / 120) * factor, 0, 100);
    familiarity.decay_debt = Math.max(0, familiarity.decay_debt - allocated * 6);
    remaining -= allocated;
  }
}
...
export function applyTrackBRecovery(save: GameSavePayload, idol: Record<string, unknown>): void {
  ...
  const m = tb.members[String(idol.uid ?? "")];
  if (!m) return;
  applySleepRecovery(m, num(idol.morale, 70), 8);
...
function generateExternalOffers(save: GameSavePayload, tb: TrackBState, iso: string): void {
  ...
  const hasPr = tb.staff.dedicated_pr;
...
export function runSundayResolution(save: GameSavePayload, iso: string): void {
  ...
  // Staff AP is calendar-bound accounting.  Unused dedicated-coach time
  // converts to role-specific production support; manager time does not.
  void usableManagerAp(tb.staff, Object.keys(tb.members).length);
  for (const kind of ["vocal", "dance"] as const) {
    const unused = Math.max(0, tb.staff[`${kind}_coach_count`] * 80 - tb.staff.coach_ap_used_week[kind]);
    tb.staff.coach_rp[kind] += unused;
    tb.staff.coach_ap_used_week[kind] = 0;
  }

  for (const familiarity of Object.values(tb.song_familiarity)) {
    const performedThisWeek = familiarity.last_performed_on != null && Date.parse(`${iso}T12:00:00Z`) - Date.parse(`${familiarity.last_performed_on}T12:00:00Z`) <= 7 * 86400000;
    const decay = familiarity.formation * .02 * (performedThisWeek ? .5 : 1);
    familiarity.formation = clamp(familiarity.formation - decay, 0, 100);
    familiarity.decay_debt += decay;
  }
  tb.staff.manager_ap_used_week = 0;
