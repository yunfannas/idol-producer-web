...
import { applyConditionCost, clamp, conditionPenalty, ensureTrackB, isoDatePart, num, recoverCondition, syncCondition } from "./runtimeState";
...
/** Themes remain semantic song metadata; they carry no runtime EXP or bonus. */
function teamThemeExecution(_tb: TrackBState, _members: MemberRuntimeState[], _themes: ThemeTag[], _segment: Segment): number { return 1; }
...
export function resolveLiveTrackB(save: GameSavePayload, group: Record<string, unknown>, idols: Record<string, unknown>[], songs: Record<string, unknown>[], live: Record<string, unknown>, _managedSongStatus?: Record<string, ManagedSongStatusRow>): LiveResultPayload {
  ...
  const setlist = resolveSetlistSongs(live, songs, String(group.uid ?? ""));
  ...
  for (const [index, song] of setlist.entries()) {
    ...
    const fam = tb.song_familiarity[uid] ?? { formation: 50, decay_debt: 0, last_performed_on: null }; const leads = assignLeads(members, profile); const values: number[] = [];
    for (const member of members) {
      ...
      const formationFactor = .9 + fam.formation / 100 * .1;
      const vocal = (0.65 * fit(breath - profile.vocal_difficulty) + .35 * fit(rhythm - profile.vocal_difficulty)) * oneSidedGap(profile.vocal_difficulty - member.attributes.pitch) * formationFactor + Math.max(0, member.attributes.tone - 10) * .012;
      const dance = fit(rhythm - profile.dance_difficulty) * oneSidedGap(profile.dance_difficulty - member.attributes.agility) * formationFactor + Math.max(0, power - 10) * .012 + (leads.dancers.has(member.idol_uid) ? .04 : 0);
      ...
      const vocalWork = 1.5 * (profile.vocal_difficulty / 12) ** 1.5;
      const danceWork = 1.5 * (profile.dance_difficulty / 12) ** 1.5;
      const roleLoad = leads.singers.has(member.idol_uid) || leads.dancers.has(member.idol_uid) ? 1.1 : .9;
      applyConditionCost(member, (2 + vocalWork + danceWork) * roleLoad); maybeIssue(member, vocalWork, danceWork, `${uid}|${member.idol_uid}|${index}`, refIso);
    }
    ...
    const familiarity = tb.song_familiarity[uid] ?? { formation: 0, decay_debt: 0, last_performed_on: null };
    familiarity.formation = clamp(familiarity.formation + .5 * (1 - familiarity.formation / 120), 0, 100);
    familiarity.last_performed_on = refIso;
    tb.song_familiarity[uid] = familiarity;
  }
  ...
  const liveSlots = Math.ceil(Math.max(60, num(live.duration_minutes, 120)) / 30);
  const liveStaffAp = liveSlots * Math.max(1, Math.ceil(members.length / 4)) + (tokutenkaiEnabled ? members.length * 2 : 0);
  tb.staff.manager_ap_used_week += liveStaffAp;
  ...
  const rows = idols.map((idol) => { const m = tb.members[String(idol.uid ?? "")]; const scores = m ? memberScores.get(m.idol_uid) ?? [.5] : [.5]; const score = scores.reduce((a, b) => a + b, 0) / scores.length; if (m) syncCondition(idol, m); return { uid: idol.uid, name: idol.name, score: Math.round(score * 100), rating: Math.round((4.2 + score * 5.2) * 100) / 100, mood_score: num(idol.morale, 70), condition_score: m?.condition ?? 90, fatigue_score: 100 - (m?.condition ?? 90), vocal_issue: m?.vocal_issue?.severity ?? null, physical_issue: m?.physical_issue?.severity ?? null, tokutenkai_sales_score: 6, work_rate: m ? (m.condition / 100) : 1 }; });
