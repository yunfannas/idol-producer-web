...
export type TraitDomain = "mc" | "acting" | "lyric_writing" | "composition" | "choreography" | "costume_design" | "producer" | "singer" | "dancer" | "comedy" | "model";
...
export interface SongRuntimeFamiliarity {
  /** Formation Familiarity, 0–100; old vocal/dance values are migration-only. */
  formation: number;
  decay_debt: number;
...
export interface MemberRuntimeState {
  ...
  /** Internal progress: crossing ±100 changes the corresponding attribute. */
  attribute_exp: Partial<Record<VisibleAttributeV2, number>>;
  /** Professional specialization outside the 17 core attributes. */
  trait_xp: Partial<Record<TraitDomain, number>>;
  trait_focus: TraitDomain | null;
  /** The only continuous short-term workload state: 100 fresh, 0 exhausted. */
  condition: number;
  /** Reversible risk accumulator derived from morning Condition, not fatigue. */
  fatigue_debt: number;
...
export interface StaffPackageState {
  team_tier: string;
  manager_count: number;
  vocal_coach_count: number;
  dance_coach_count: number;
  dedicated_pr: boolean;
  manager_ap_used_week: number;
  coach_ap_used_week: { vocal: number; dance: number };
  coach_rp: { vocal: number; dance: number };
}
